...
- **No mutual reveal without a match** — a user never learns that someone passed on
  or liked them; identities/intent are revealed only on a mutual match.
...
> Functional Requirements list below is the acceptance surface.

# TODO: Given/When/Then user stories — see Open Questions
...
1. **What are the explicit Given/When/Then user stories?** — The user opted to skip
   them for the MVP and use the Functional Requirements as the acceptance surface.
   Owner: user. Resolve if downstream review or implementation needs per-story
   acceptance criteria. Block: no.
2. **What is the exact hard deadline (hackathon date)?** — `hard_deadline` recorded
   as `null`; shape-notes flags the fixed hackathon date as TBD. Owner: user.
   By: before scheduling implementation work.
