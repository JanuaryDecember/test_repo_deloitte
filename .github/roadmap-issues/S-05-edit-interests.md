...
| Field | Value |
| --- | --- |
| Stream | Stream C - Profile maintenance |
| Type | slice |
| PRD refs | FR-007 |
| Status | proposed |
...
### Why / risk
Nice-to-have (FR-007) - PRD says build only if the core flow is done. Low risk; runs on its own branch off S-02 and blocks nothing. Under the speed goal it is the first thing to drop if time runs short.
